...
Apps generated by the harness live outside this repo under `~/src/kodr-testing`, organized by phase number and slug (for example `~/src/kodr-testing/phase-109/brownfield-fix-1`). Keeping them out of the repo keeps generated code and its `.kodr` run artifacts from polluting the harness' own context and test suite.

These are Kodr samples with provenance, not hand-written fixtures — every example is generated by running Kodr itself against the local model, and the run logs feed the development of Kodr.
