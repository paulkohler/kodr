...
// Commit applied writes when --commit is set and the run is in a committable
// state: writes applied, no run errors, and tests (when run) passing. Returns
// null when --commit was not requested, otherwise an honest record of what
// happened for git.json and the run summary.
