...
export function parseArgs(argv, env = {}) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--base-url' || arg === '--model' || arg === '--api-key' || arg === '--timeout-ms') {
...
async function probe(options, io) {
  ...
  if (!model) {
    throw new CliError('No model was provided and GET /models did not return a usable model id');
  }
  ...
  const chatResponse = await requestJson(`${options.baseUrl}/chat/completions`, {
    apiKey: options.apiKey,
    body: chatBody,
    method: 'POST',
    timeoutMs: options.timeoutMs
  ...
  if (!reply) {
    throw new CliError('POST /chat/completions did not return a usable assistant message');
  }
...
async function requestJson(url, options) {
  ...
  if (!response.ok) {
    throw new CliError(`${options.method} ${url} returned HTTP ${response.status}`);
  }
...
function firstModelId(body) {
  ...
  const model = body.data.find((item) => item && typeof item.id === 'string' && item.id.length > 0);
