...
- `kodr run` and `kodr tui` default to favouring work: tools on, writes applied, and detected tests run automatically (writes are git-aware and undoable). Opt out with `--dry-run` (propose only), `--no-tools`, `--no-test`, `--confirm` (y/N apply prompt on a TTY run), or in-session `/tools off` / `/apply off`. `--json` stays explicit for scripting (dry unless `--yes`).
