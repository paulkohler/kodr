...
- One-shot `run` defaults file writes to dry-run. The interactive `kodr tui` defaults to tools-on and apply-on (the explicit apply path this rule waited for) so a session is useful straight up; disable with `--no-tools` / `--dry-run` or in-session `/tools off` / `/apply off`.
