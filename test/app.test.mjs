...
describe('usage', () => {
	...
	it('describes apply-by-default run behavior without the obsolete prompt claim', () => {
		const text = usage();
		assert.match(text, /applies writes and runs detected tests/u);
		assert.match(text, /--confirm\s+# TTY: prompts apply/u);
		assert.doesNotMatch(text, /TTY: prompts apply\? \[y\/N\] before writing/u);
	});
});
