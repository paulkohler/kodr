...
describe('probe', () => {
  it('calls OpenAI-compatible endpoints and writes run artifacts', async () => {
    ...
    try {
      ...
      const artifact = JSON.parse(await readFile(join(output.runDir, 'result.json'), 'utf8'));
      assert.equal(artifact.ok, true);
      assert.equal(artifact.reply, 'koder-probe-ok');

      const chatResponse = JSON.parse(await readFile(join(output.runDir, 'chat-response.json'), 'utf8'));
